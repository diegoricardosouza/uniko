...
import { useFinalitiesController } from "../useFinalitiesController";
import { useColumnsFinalities } from "./useColumnsFinalities";
...
export function FinalitiesDataTable({ finalities, isLoading }: UsersDataTableProps) {
  ...
  const { 
    ...
    handleNew,
    handleDelete
  } = useFinalitiesController()

  const columnsUser = useColumnsFinalities(handleDelete, handleEdit);
      ...
      <div className="flex">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-md" onOpenAutoFocus={(e) => e.preventDefault()} onCloseAutoFocus={(e) => e.preventDefault()}>
            <DialogHeader>
              <DialogTitle>
                {editingCategory ? "Editar Finalidade" : "Nova Finalidade"}
              </DialogTitle>
              <DialogDescription>
                {editingCategory
                  ? "Atualize as informações da categoria"
                  : "Crie uma nova finalidade para seus imóveis"
                }
              </DialogDescription>
            </DialogHeader>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          ...
                          <Input placeholder="Digite a finalidade" className="pl-10" {...field} />
                        </div>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
      ...
      <div className="w-full min-h-[300px] relative overflow-hidden">
        ...
        <div>
          <div className="flex items-center justify-between gap-2 py-4">
            <Input
              placeholder="Filtrar finalidades..."
              value={globalFilter}
              onChange={(event) => setGlobalFilter(event.target.value)}
              className="max-w-sm"
            />

            <span className="text-sm">{finalities.length} itens</span>
          </div>
