...
export function CardPost({ post }: CardPostProps) {
  const featuredImageUrl = post.medias?.filter((media) => media.mediaType === 'featured_image')[0]?.url;
  const category = post.categories?.[0]?.name;

  return (
    <article>
      <div className="border-b border-gold pb-[30px]">
        <Image
        ...
        <p className="font-inter text-[17px] text-content leading-[20px] font-normal mb-[15px]">
          {trimWords(post.subtitle! || post.content!, 16)}
        </p>

        <div className="flex justify-center">
          <Link href={`/imovel/${post.slug!}`} className="button-geral !text-white shadow-[0px_3px_6px_#00000029] !bg-black hover:!bg-gold">
            CONTINUE LENDO
