  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      if (multiple && maxFiles !== Number.POSITIVE_INFINITY && state.files.length + newFilesArray.length > maxFiles) {
        errors.push(`Você só pode fazer upload de no máximo ${maxFiles} arquivos.`)
        setState((prev) => ({ ...prev, errors }))
        return
      }
  ...
  const reorderFiles = useCallback(
    (startIndex: number, endIndex: number) => {
      setState((prev) => {
        ...
        const [reorderedItem] = newFiles.splice(startIndex, 1)
        newFiles.splice(endIndex, 0, reorderedItem)
